...
5.  What data types are contained in the variables? (Reminder: [https://rbtl-fs24.github.io/website/slides/lec-02-visualisation.html#/types-of-variables](https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables){.uri})
    ...
    species: non-numerical (string)

    island: non-numerical (string)

    bill_length_mm: numerical, continuous (float)

    bill_depth_mm: numerical, continuous (float)

    flipper_length_mm: numerical, discrete (int)

    body_mass_g: numerical, discrete (int)

    sex: non-numerical (string)

    year: numerical, discrete (int)
...
penguins |> 
  filter(sex == "female", species == "Chinstrap", island %in% c("Dream", "Torgensen"))
```
...
penguins |> 
  filter(sex == "female", island %in% c("Torgensen", "Biscoe")) |> 
  count(sex, island)
```
